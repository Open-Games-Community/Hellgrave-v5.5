...
local player_positions = {
    [1] = {fromPos = Position(31941, 32012, 7), toPos = Position(31938, 31990, 6)},
    [2] = {fromPos = Position(31942, 32012, 7), toPos = Position(31939, 31990, 6)},
    [3] = {fromPos = Position(31943, 32012, 7), toPos = Position(31940, 31990, 6)},
    [4] = {fromPos = Position(31944, 32012, 7), toPos = Position(31941, 31990, 6)}
}

local monsters = {
    [1] = {pos = Position(31942, 31998, 6), name = "Medusa"},
    [2] = {pos = Position(31947, 31995, 6), name = "Medusa"}
}
local quest_range = {fromPos = Position(31931, 31991, 6), toPos = Position(31950, 32005, 6)} 

local exit_position = Position(32215, 32268, 6) -- Position completely outside the quest area

local earthArenaSecond = Action()
...
local specs, spec = Game.getSpectators(Position(33919, 31646, 8), false, false, 18, 18, 18, 18) -- Do not change
    for j = 1, #specs do
        spec = specs[j]
        if spec:getName():lower() == 'Medusa' then
            spec:remove()
        end
    end
end

function earthArenaSecond.onUse(player, item, fromPosition, target, toPosition, isHotkey)
...
earthArenaSecond:uid(41387)
earthArenaSecond:register()
