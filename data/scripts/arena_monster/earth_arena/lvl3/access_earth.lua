...
local player_positions = {
    [1] = {fromPos = Position(31929, 32012, 6), toPos = Position(31951, 31987, 5)},
    [2] = {fromPos = Position(31930, 32012, 6), toPos = Position(31952, 31987, 5)},
    [3] = {fromPos = Position(31931, 32012, 6), toPos = Position(31953, 31987, 5)},
    [4] = {fromPos = Position(31932, 32012, 6), toPos = Position(31954, 31987, 5)}
}

local monsters = {
    [1] = {pos = Position(31947, 31999, 5), name = "Ripper Spectre"},
    [2] = {pos = Position(31957, 31999, 5), name = "Ripper Spectre"},
    [2] = {pos = Position(31954, 32001, 5), name = "Ripper Spectre"}
}
local quest_range = {fromPos = Position(31943, 31997, 5), toPos = Position(31960, 32005, 5)} 

local exit_position = Position(32215, 32268, 6) -- Position completely outside the quest area

local earthArenaThird = Action()
...
local specs, spec = Game.getSpectators(Position(33919, 31646, 8), false, false, 18, 18, 18, 18) -- Do not change
    for j = 1, #specs do
        spec = specs[j]
        if spec:getName():lower() == 'Ripper Spectre' then
            spec:remove()
        end
    end
end

function earthArenaThird.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    for i = 1, #player_positions do
        ...
        if not tile then
            print(">> ERROR: Arena Earth tile does not exist for Position(" .. fromPos.x .. ", " .. fromPos.y .. ", " .. fromPos.z .. ").")
    ...
    for i = 1, #monsters do
        ...
        if not Tile(toPos) then
            print(">> ERROR: Arena Earth tile does not exist for Position(" .. toPos.x .. ", " .. toPos.y .. ", " .. toPos.z .. ").")
            return player:sendCancelMessage("There is an issue with this quest. Please contact an administrator.")
        end
        removeBoss()
...
earthArenaThird:uid(41388)
earthArenaThird:register()
