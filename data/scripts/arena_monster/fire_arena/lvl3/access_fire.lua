...
local player_positions = {
    [1] = {fromPos = Position(31947, 31956, 6), toPos = Position(31946, 31944, 5)},
    [2] = {fromPos = Position(31947, 31957, 6), toPos = Position(31947, 31944, 5)},
    [3] = {fromPos = Position(31947, 31958, 6), toPos = Position(31948, 31944, 5)},
    [4] = {fromPos = Position(31947, 31959, 6), toPos = Position(31949, 31944, 5)}
}

local monsters = {
    [1] = {pos = Position(31946, 31953, 5), name = "Vexclaw"},
    [2] = {pos = Position(31952, 31956, 5), name = "Hellflayer"}
}
local quest_range = {fromPos = Position(31944, 31950, 5), toPos = Position(31953, 31959, 5)} 

local exit_position = Position(32215, 32268, 6) -- Position completely outside the quest area

local fireArenaThird = Action()
...
local specs, spec = Game.getSpectators(Position(33919, 31646, 8), false, false, 18, 18, 18, 18) -- Do not change
    for j = 1, #specs do
        spec = specs[j]
        if spec:getName():lower() == 'Vexclaw' then
            spec:remove()
        end
    end
end

function fireArenaThird.onUse(player, item, fromPosition, target, toPosition, isHotkey)
...
fireArenaThird:uid(41385)
fireArenaThird:register()
