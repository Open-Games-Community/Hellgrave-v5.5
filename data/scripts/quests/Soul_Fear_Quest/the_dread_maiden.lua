...
local player_positions = {
    [1] = {fromPos = Position(32835, 31816, 8), toPos = Position(32807, 31816, 8)},
    [2] = {fromPos = Position(32836, 31816, 8), toPos = Position(32807, 31817, 8)},
    [3] = {fromPos = Position(32837, 31816, 8), toPos = Position(32807, 31818, 8)},
    [4] = {fromPos = Position(32838, 31816, 8), toPos = Position(32807, 31819, 8)}
}

local monsters = {
    [1] = {pos = Position(32808, 31812, 8), name = "The Dread Maiden"}
}
local quest_range = {fromPos = Position(32803, 31807, 8), toPos = Position(32815, 31820, 8)} -- see image in thread for explanation

local exit_position = Position(32841, 31816, 8) -- Position completely outside the quest area

local theDreadMaiden = Action()
...
local specs, spec = Game.getSpectators(Position(33919, 31646, 8), false, false, 18, 18, 18, 18)
    for j = 1, #specs do
        spec = specs[j]
        if spec:getName():lower() == 'The Dread Maiden' then
            spec:remove()
        end
    end
end

function theDreadMaiden.onUse(player, item, fromPosition, target, toPosition, isHotkey)
...
theDreadMaiden:uid(42605)
theDreadMaiden:register()