...
local player_positions = {
    [1] = {fromPos = Position(32374, 31599, 8), toPos = Position(32351, 31582, 8)},
    [2] = {fromPos = Position(32374, 31598, 8), toPos = Position(32351, 31581, 8)},
    [3] = {fromPos = Position(32374, 31597, 8), toPos = Position(32351, 31580, 8)},
    [4] = {fromPos = Position(32374, 31600, 8), toPos = Position(32351, 31583, 8)},
	[5] = {fromPos = Position(32374, 31601, 8), toPos = Position(32351, 31584, 8)},
	[6] = {fromPos = Position(32375, 31599, 8), toPos = Position(32352, 31582, 8)},
	[7] = {fromPos = Position(32375, 31598, 8), toPos = Position(32352, 31581, 8)},
	[8] = {fromPos = Position(32375, 31597, 8), toPos = Position(32352, 31580, 8)},
	[9] = {fromPos = Position(32375, 31600, 8), toPos = Position(32352, 31583, 8)},
	[10] = {fromPos = Position(32375, 31601, 8), toPos = Position(32352, 31584, 8)}
}

local monsters = {
    [1] = {pos = Position(32332, 31599, 8), name = "King Zelos"}
}
local quest_range = {fromPos = Position(32326, 31591, 8), toPos = Position(32338, 31606, 8)} -- see image in thread for explanation

local exit_position = Position(32379, 31598, 8) -- Position completely outside the quest area
...
local specs, spec = Game.getSpectators(Position(33919, 31646, 8), false, false, 18, 18, 18, 18)
    for j = 1, #specs do
        spec = specs[j]
        if spec:getName():lower() == 'King Zelos' then
            spec:remove()
        end
    end
end
...
function lever.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    for i = 1, #player_positions do
        ...
        if not tile then
            print(">> ERROR: King Zelos tile does not exist for Position(" .. fromPos.x .. ", " .. fromPos.y .. ", " .. fromPos.z .. ").")
    ...
    for i = 1, #monsters do
        ...
        if not Tile(toPos) then
            print(">> ERROR: King Zelos tile does not exist for Position(" .. toPos.x .. ", " .. toPos.y .. ", " .. toPos.z .. ").")
            return player:sendCancelMessage("There is an issue with this quest. Please contact an administrator.")
        end
        removeBoss()
    ...
    for i = 1, #participants do
        ...
        player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You have 60 minutes to kill and loot this boss. Otherwise you will lose that chance and will be kicked out.")
    end
