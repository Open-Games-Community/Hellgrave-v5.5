...
local player_positions = {
    [1] = {fromPos = Position(32344, 31466, 8), toPos = Position(32343, 31485, 8)},
    [2] = {fromPos = Position(32345, 31466, 8), toPos = Position(32344, 31485, 8)},
    [3] = {fromPos = Position(32346, 31466, 8), toPos = Position(32345, 31485, 8)},
    [4] = {fromPos = Position(32347, 31466, 8), toPos = Position(32346, 31485, 8)},
	[5] = {fromPos = Position(32348, 31466, 8), toPos = Position(32347, 31485, 8)}
}

local monsters = {
    [1] = {pos = Position(32345, 31490, 8), name = "Count Vlarkorth"}
}
local quest_range = {fromPos = Position(32341, 31483, 8), toPos = Position(32350, 31497, 8)} -- see image in thread for explanation

local exit_position = Position(32345, 31460, 8) -- Position completely outside the quest area
...
local specs, spec = Game.getSpectators(Position(33919, 31646, 8), false, false, 18, 18, 18, 18)
    for j = 1, #specs do
        spec = specs[j]
        if spec:getName():lower() == 'Count Vlarkorth' then
            spec:remove()
        end
    end
end
...
function lever.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    for i = 1, #player_positions do
        ...
        if not tile then
            print(">> ERROR: Count vlarkorth tile does not exist for Position(" .. fromPos.x .. ", " .. fromPos.y .. ", " .. fromPos.z .. ").")
    ...
    for i = 1, #monsters do
        ...
        if not Tile(toPos) then
            print(">> ERROR: Count vlarkorth tile does not exist for Position(" .. toPos.x .. ", " .. toPos.y .. ", " .. toPos.z .. ").")
            return player:sendCancelMessage("There is an issue with this quest. Please contact an administrator.")
        end
        removeBoss()
    ...
    addEvent(doResetTheBossDukeKrule, config.duration * 60 * 100,  toPosition, cid_array)
    return true
end
