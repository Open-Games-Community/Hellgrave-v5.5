...
local player_positions = {
    [1] = {fromPos = Position(32313, 31466, 8), toPos = Position(32312, 31488, 8)},
    [2] = {fromPos = Position(32314, 31466, 8), toPos = Position(32313, 31488, 8)},
    [3] = {fromPos = Position(32315, 31466, 8), toPos = Position(32314, 31488, 8)},
    [4] = {fromPos = Position(32316, 31466, 8), toPos = Position(32315, 31488, 8)},
	[5] = {fromPos = Position(32317, 31466, 8), toPos = Position(32316, 31488, 8)}
}

local monsters = {
    [1] = {pos = Position(32312, 31492, 8), name = "Sir Baeloc"},
	[2] = {pos = Position(32312, 31492, 8), name = "Sir Nictros"}
}
local quest_range = {fromPos = Position(32308, 31486, 8), toPos = Position(32319, 31500, 8)} -- see image in thread for explanation

local exit_position = Position(32315, 31461, 8) -- Position completely outside the quest area
...
local specs, spec = Game.getSpectators(Position(33919, 31646, 8), false, false, 18, 18, 18, 18)
    for j = 1, #specs do
        spec = specs[j]
        if spec:getName():lower() == 'Sir Baeloc' then
            spec:remove()
        end
    end
end
...
function lever.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    for i = 1, #player_positions do
        ...
        if not tile then
            print(">> ERROR: Sir Nictros tile does not exist for Position(" .. fromPos.x .. ", " .. fromPos.y .. ", " .. fromPos.z .. ").")
    ...
    for i = 1, #monsters do
        ...
        if not Tile(toPos) then
            print(">> ERROR: Sir Nictros tile does not exist for Position(" .. toPos.x .. ", " .. toPos.y .. ", " .. toPos.z .. ").")
            return player:sendCancelMessage("There is an issue with this quest. Please contact an administrator.")
        end
        removeBoss()
