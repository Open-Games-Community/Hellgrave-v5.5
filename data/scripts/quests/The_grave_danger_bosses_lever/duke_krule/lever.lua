...
local player_positions = {
    [1] = {fromPos = Position(32344, 31546, 8), toPos = Position(32344, 31531, 8)},
    [2] = {fromPos = Position(32345, 31546, 8), toPos = Position(32345, 31531, 8)},
    [3] = {fromPos = Position(32346, 31546, 8), toPos = Position(32346, 31531, 8)},
    [4] = {fromPos = Position(32347, 31546, 8), toPos = Position(32347, 31531, 8)},
	[5] = {fromPos = Position(32348, 31546, 8), toPos = Position(32348, 31531, 8)}
}

local monsters = {
    [1] = {pos = Position(32345, 31524, 8), name = "Duke Krule"}
}
local quest_range = {fromPos = Position(32341, 31518, 8), toPos = Position(32349, 31533, 8)} -- see image in thread for explanation

local exit_position = Position(32346, 31551, 8) -- Position completely outside the quest area
...
local specs, spec = Game.getSpectators(Position(33919, 31646, 8), false, false, 18, 18, 18, 18)
    for j = 1, #specs do
        spec = specs[j]
        if spec:getName():lower() == 'Duke Krule' then
            spec:remove()
        end
    end
end
...
function lever.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    for i = 1, #player_positions do
        ...
        if not tile then
            print(">> ERROR: Duke Krule tile does not exist for Position(" .. fromPos.x .. ", " .. fromPos.y .. ", " .. fromPos.z .. ").")
    ...
    for i = 1, #monsters do
        ...
        if not Tile(toPos) then
            print(">> ERROR: Duke Krule tile does not exist for Position(" .. toPos.x .. ", " .. toPos.y .. ", " .. toPos.z .. ").")
            return player:sendCancelMessage("There is an issue with this quest. Please contact an administrator.")
        end
        removeBoss()
