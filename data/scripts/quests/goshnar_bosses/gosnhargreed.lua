...
local player_positions = {
    [1] = {fromPos = Position(32522, 31632, 8), toPos = Position(32490, 31629, 8)},
    [2] = {fromPos = Position(32523, 31632, 8), toPos = Position(32491, 31629, 8)},
    [3] = {fromPos = Position(32524, 31632, 8), toPos = Position(32492, 31629, 8)},
    [4] = {fromPos = Position(32525, 31632, 8), toPos = Position(32493, 31629, 8)}
}

local monsters = {
    [1] = {pos = Position(32492, 31633, 8), name = "Goshnar Greed"}
}
local quest_range = {fromPos = Position(32487, 31626, 8), toPos = Position(32497, 31639, 8)} -- see image in thread for explanation

local exit_position = Position(32528, 31632, 8) -- Position completely outside the quest area
...
local specs, spec = Game.getSpectators(Position(33919, 31646, 8), false, false, 18, 18, 18, 18)
    for j = 1, #specs do
        spec = specs[j]
        if spec:getName():lower() == 'Many Faces' then
            spec:remove()
        end
    end
end

function goshnarGreed.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    for i = 1, #player_positions do
        ...
        if not tile then
            print(">> ERROR: Goshnar Greed tile does not exist for Position(" .. fromPos.x .. ", " .. fromPos.y .. ", " .. fromPos.z .. ").")
    ...
    for i = 1, #monsters do
        ...
        if not Tile(toPos) then
            print(">> ERROR: Goshnar Greed tile does not exist for Position(" .. toPos.x .. ", " .. toPos.y .. ", " .. toPos.z .. ").")
            return player:sendCancelMessage("There is an issue with this quest. Please contact an administrator.")
        end
        removeBoss()
