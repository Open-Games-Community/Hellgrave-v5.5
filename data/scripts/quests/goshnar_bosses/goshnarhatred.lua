...
local player_positions = {
    [1] = {fromPos = Position(32519, 31568, 8), toPos = Position(32487, 31563, 8)},
    [2] = {fromPos = Position(32520, 31568, 8), toPos = Position(32487, 31563, 8)},
    [3] = {fromPos = Position(32521, 31568, 8), toPos = Position(32487, 31563, 8)},
    [4] = {fromPos = Position(32522, 31568, 8), toPos = Position(32487, 31563, 8)}
}

local monsters = {
    [1] = {pos = Position(32489, 31567, 8), name = "Goshnar Hatred"}
}
local quest_range = {fromPos = Position(32484, 31561, 8), toPos = Position(32495, 31572, 8)} -- see image in thread for explanation

local exit_position = Position(32520, 31566, 8) -- Position completely outside the quest area
...
function goshnarHatred.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    for i = 1, #player_positions do
        ...
        if not tile then
            print(">> ERROR: Goshnar Hatred tile does not exist for Position(" .. fromPos.x .. ", " .. fromPos.y .. ", " .. fromPos.z .. ").")
    ...
    for i = 1, #monsters do
        ...
        if not Tile(toPos) then
            print(">> ERROR: Goshnar Hatred tile does not exist for Position(" .. toPos.x .. ", " .. toPos.y .. ", " .. toPos.z .. ").")
            return player:sendCancelMessage("There is an issue with this quest. Please contact an administrator.")
        end
        removeBoss()
