...
local player_positions = {
    [1] = {fromPos = Position(32422, 31601, 8), toPos = Position(32454, 31596, 8)},
    [2] = {fromPos = Position(32423, 31601, 8), toPos = Position(32455, 31596, 8)},
    [3] = {fromPos = Position(32424, 31601, 8), toPos = Position(32456, 31596, 8)},
    [4] = {fromPos = Position(32425, 31601, 8), toPos = Position(32457, 31596, 8)}
}

local monsters = {
    [1] = {pos = Position(32455, 31601, 8), name = "Goshnar Megalomania"}
}
local quest_range = {fromPos = Position(32450, 31594, 8), toPos = Position(32461, 31607, 8)} -- see image in thread for explanation

local exit_position = Position(32426, 31599, 8) -- Position completely outside the quest area
...
local specs, spec = Game.getSpectators(Position(33919, 31646, 8), false, false, 18, 18, 18, 18)
    for j = 1, #specs do
        spec = specs[j]
        if spec:getName():lower() == 'Goshnar Megalomania' then
            spec:remove()
        end
    end
end

function goshnarMegalomania.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    for i = 1, #player_positions do
        ...
        if not tile then
            print(">> ERROR: Goshnar Megalomania tile does not exist for Position(" .. fromPos.x .. ", " .. fromPos.y .. ", " .. fromPos.z .. ").")
    ...
    for i = 1, #monsters do
        ...
        if not Tile(toPos) then
            print(">> ERROR: Goshnar Megalomania tile does not exist for Position(" .. toPos.x .. ", " .. toPos.y .. ", " .. toPos.z .. ").")
            return player:sendCancelMessage("There is an issue with this quest. Please contact an administrator.")
        end
        removeBoss()
