...
local player_positions = {
    [1] = {fromPos = Position(33042, 32181, 6), toPos = Position(33040, 32187, 7)},
    [2] = {fromPos = Position(33043, 32181, 6), toPos = Position(33041, 32187, 7)},
    [3] = {fromPos = Position(33044, 32181, 6), toPos = Position(33042, 32187, 7)},
    [4] = {fromPos = Position(33045, 32181, 6), toPos = Position(33043, 32187, 7)}
}

local monsters = {
    [1] = {pos = Position(33043, 32192, 7), name = "Goshnar Cruelty"}
}
local quest_range = {fromPos = Position(33038, 32186, 7), toPos = Position(33050, 32199, 7)} -- see image in thread for explanation
...
local specs, spec = Game.getSpectators(Position(33919, 31646, 8), false, false, 18, 18, 18, 18)
    for j = 1, #specs do
        spec = specs[j]
        if spec:getName():lower() == 'Goshnar Cruelty' then
            spec:remove()
        end
    end
end
...
function goshnarCruelty.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    for i = 1, #player_positions do
        ...
        if not tile then
            print(">> ERROR: Goshnar Cruelty tile does not exist for Position(" .. fromPos.x .. ", " .. fromPos.y .. ", " .. fromPos.z .. ").")
    ...
    for i = 1, #monsters do
        ...
        if not Tile(toPos) then
            print(">> ERROR: Goshnar Cruelty tile does not exist for Position(" .. toPos.x .. ", " .. toPos.y .. ", " .. toPos.z .. ").")
            return player:sendCancelMessage("There is an issue with this quest. Please contact an administrator.")
        end
        removeBoss()
