...
local player_positions = {
    [1] = {fromPos = Position(32520, 31601, 8), toPos = Position(32487, 31596, 8)},
    [2] = {fromPos = Position(32521, 31601, 8), toPos = Position(32488, 31596, 8)},
    [3] = {fromPos = Position(32522, 31601, 8), toPos = Position(32489, 31596, 8)},
    [4] = {fromPos = Position(32523, 31601, 8), toPos = Position(32490, 31596, 8)}
}

local monsters = {
    [1] = {pos = Position(32488, 31600, 8), name = "Goshnar Spite"}
}
local quest_range = {fromPos = Position(32482, 31593, 8), toPos = Position(32495, 31606, 8)} -- see image in thread for explanation

local exit_position = Position(32526, 31601, 8) -- Position completely outside the quest area
...
local specs, spec = Game.getSpectators(Position(33919, 31646, 8), false, false, 18, 18, 18, 18)
    for j = 1, #specs do
        spec = specs[j]
        if spec:getName():lower() == 'Goshnar Spite' then
            spec:remove()
        end
    end
end

function goshnarSpite.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    for i = 1, #player_positions do
        ...
        if not tile then
            print(">> ERROR: Goshnar Spite tile does not exist for Position(" .. fromPos.x .. ", " .. fromPos.y .. ", " .. fromPos.z .. ").")
    ...
    for i = 1, #monsters do
        ...
        if not Tile(toPos) then
            print(">> ERROR: Goshnar Spite tile does not exist for Position(" .. toPos.x .. ", " .. toPos.y .. ", " .. toPos.z .. ").")
            return player:sendCancelMessage("There is an issue with this quest. Please contact an administrator.")
        end
        removeBoss()
