...
local player_positions = {
    [1] = {fromPos = Position(32425, 31566, 8), toPos = Position(32453, 31560, 8)},
    [2] = {fromPos = Position(32426, 31566, 8), toPos = Position(33454, 31560, 8)},
    [3] = {fromPos = Position(32427, 31566, 8), toPos = Position(33455, 31560, 8)},
    [4] = {fromPos = Position(32428, 31566, 8), toPos = Position(33456, 31560, 8)}
}

local monsters = {
    [1] = {pos = Position(32455, 31565, 8), name = "Goshnar Malice"}
}
local quest_range = {fromPos = Position(32449, 31560, 8), toPos = Position(32460, 31573, 8)} -- see image in thread for explanation

local exit_position = Position(32427, 31564, 8) -- Position completely outside the quest area
...
local specs, spec = Game.getSpectators(Position(33919, 31646, 8), false, false, 18, 18, 18, 18)
    for j = 1, #specs do
        spec = specs[j]
        if spec:getName():lower() == 'Goshnar Malice' then
            spec:remove()
        end
    end
end
...
function goshnarMalice.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    for i = 1, #player_positions do
        ...
        if not tile then
            print(">> ERROR: Goshnar malice tile does not exist for Position(" .. fromPos.x .. ", " .. fromPos.y .. ", " .. fromPos.z .. ").")
    ...
    for i = 1, #monsters do
        ...
        if not Tile(toPos) then
            print(">> ERROR: Goshnar Malice tile does not exist for Position(" .. toPos.x .. ", " .. toPos.y .. ", " .. toPos.z .. ").")
            return player:sendCancelMessage("There is an issue with this quest. Please contact an administrator.")
        end
        removeBoss()
